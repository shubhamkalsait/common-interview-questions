...
1️⃣ WHAT
IAM is the AWS service that controls who can access your AWS account and what they can do.
...
3️⃣ WHY
You need a central place to securely control access across many services and accounts.
It enables least privilege, separating dev/qa/prod access, and reducing blast radius in production.
It removes the need to hardcode credentials by using roles and temporary credentials, which is critical for security and compliance.
4️⃣ HOW
Conceptually:
...
1️⃣ WHAT
S3 storage classes are cost-performance tiers for S3 objects that control durability, availability, access speed, and price.
...
1️⃣ WHAT
EC2 instance types are hardware templates that define vCPUs, RAM, storage options, network bandwidth, and accelerators for your virtual servers.
...
Instance families allow you to right-size workload cost and performance instead of overpaying for unnecessary resources.
...
1️⃣ WHAT
An AWS load balancer distributes incoming traffic across multiple targets (EC2, containers, IPs) to improve availability and scalability.
...
They also integrate with SSL termination, WAF, and Auto Scaling Groups to build resilient production architectures.
4️⃣ HOW
Conceptually:
...
1️⃣ WHAT
An Auto Scaling Group (ASG) automatically adjusts the number of EC2 instances based on demand and health.
One-line: An ASG is a group of EC2 instances that scales out and in automatically to match load and maintain availability.
2️⃣ TYPES
Not “types” of ASG, but scaling approaches:
...
1️⃣ WHAT
VPC Peering is a network connectivity option that connects two VPCs so that their resources can communicate privately using private IPs.
...
1️⃣ WHAT
A NAT Gateway is a managed AWS service that allows private subnet instances to access the internet (or other public services) outbound, while blocking unsolicited inbound connections.
...
1️⃣ WHAT
Both NAT Gateway and NAT Instance provide outbound internet access for private subnet resources, but one is managed service, the other is a self-managed EC2 instance.
